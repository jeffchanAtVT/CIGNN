...
def masked_mae_loss(null_val, scaler=None, multiple=False):
    def loss(preds, labels):
        if scaler and multiple:
            mae = 0
            for i, s in enumerate(scaler):
                pred = s.inverse_transform(preds[i])
                label = s.inverse_transform(labels[i])
                mae += masked_mae_tf(preds=pred, labels=label, null_val=null_val)
            return mae
        if multiple:
            mae = 0
